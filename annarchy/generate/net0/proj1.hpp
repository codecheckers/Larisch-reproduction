        ...
        if (_transmission && pop2._active){
            ...
            for(int _idx_j = 0; _idx_j < pop1.spiked.size(); _idx_j++){
                ...
                for(int _idx_i = 0; _idx_i < nb_post; _idx_i++){
                    ...
                    pop2.g_Exc[post_rank[i]] +=  transmit*w[i][j];
        ...
        if(_transmission && _update && pop2._active && ( (t - _update_offset)%_update_period == 0L) ){
            ...
            for(int i = 0; i < post_rank.size(); i++){
                ...
                for(int j = 0; j < pre_rank[i].size(); j++){
                    ...
                    ltdTerm_fix[i][j] = (w[i][j] > wMin ? pop1.Spike[rk_pre]*aLTD*vmean_fix*positive(pop2.umeanLTD[rk_post] - thetaLTD)/urefsquare : 0.0);


                    // ltdTerm = if w>wMin : (aLTD*(post.vmean/urefsquare)*pre.Spike * pos(post.umeanLTD - thetaLTD)) else : 0.0
                    ltdTerm[i][j] = (w[i][j] > wMin ? pop2.vmean[rk_post]*pop1.Spike[rk_pre]*aLTD*positive(pop2.umeanLTD[rk_post] - thetaLTD)/urefsquare : 0.0);


                    // ltpTerm = if w<wMax : (aLTP * pos(post.vm - thetaLTP) *(pre.xtrace)* pos(post.umeanLTP - thetaLTD)) else : 0.0
                    ltpTerm[i][j] = (w[i][j] < wMax ? pop1.xtrace[rk_pre]*aLTP*positive(pop2.umeanLTP[rk_post] - thetaLTD)*positive(pop2.vm[rk_post] - thetaLTP) : 0.0);
